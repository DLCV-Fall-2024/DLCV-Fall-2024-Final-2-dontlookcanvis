...
for config_file in "$prompt1_folder"/*; do
    echo "$config_file"
    python sample.py --config_file "$config_file" --outimg_dir "$outimg_dir" --prompt_num "1_accum" --accum True
done
...
for config_file in "$prompt2_folder"/*; do
  echo "$config_file" 
  CUDA_VISIBLE_DEVICES=2 python sample.py --config_file "$config_file" --outimg_dir "$outimg_dir" --prompt_num "2_accum" --accum True
done
...
for config_file in "$prompt3_folder"/*; do
  echo "$config_file" 
  python sample_sdxl.py --config_file "$config_file" --outimg_dir "$outimg_dir" --prompt_num "3_accum" --accum True
done
...
for config_file in "$prompt1_folder"/*; do
  ...
  python3 sample.py --config_file "$config_file" --outimg_dir "$outimg_dir" --prompt_num "0" --accum True
done
...
for config_file in "$prompt1_folder"/*; do
  echo "$config_file" >> inference_accum_config_check
  python3 sample.py --config_file "$config_file" --outimg_dir "$outimg_dir" --prompt_num "1" --accum True
done
...
for config_file in "$prompt2_folder"/*; do
  ...
  python3 sample.py --config_file "$config_file" --outimg_dir "$outimg_dir" --prompt_num "2" --accum True
done

# Folder contains prompt 3's config files
prompt3_folder="./configs/inference/prompt3_exp"

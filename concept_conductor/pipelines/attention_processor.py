...
class AttentionController(object):
    def __init__(self, ops_dict, w_min=8, h_min=8,
                 ...
                 device="cuda", dtype=torch.float16, rect_mask=False, accum=False):
        self.storage = dict()
    ...
    def record_info(self, keys, value):
        par = self.kmeans_info
        for key in keys[:-1]:
            if key not in par:
                par[key] = dict()
            par = par[key]
    ...
    def choose_mask2(self, batch_masks, ref_masks=None, point=None, ref_twice_masks=None): 
        # print("choose mask2", flush=True)
        # ref_mask: [w, h]
        chosen_masks = []
        merge_indices = []
        if ref_masks is not None:
            for seed, (seed_masks, ref_mask) in enumerate(zip(batch_masks, ref_masks)):
                ...
                for i, (idx, overlap) in enumerate(values):
                    new_mask = merge_mask + seed_masks[idx]
                    new_mask[new_mask>1.] = 1.
                    m1 = new_mask * ref_mask
                    m2 = new_mask + ref_mask
                    m2[m2>1.] = 1.
                    overlap = m1.sum() / m2.sum()
                    if overlap < max_overlap:
                        continue
                ...
                if max_overlap > self.mask_overlap_threshold:
                    ...
                    # print("Merge: masks #", merge_idx, flush=True)
                    merge_indices.append((merge_idx, max_overlap.item()))
                else:
                    # print("Fail to obtain max mask. Use rect mask.", flush=True)
                    ...
                    merge_indices.append(None)
        return chosen_masks, merge_indices

    def choose_mask(self, batch_masks, ref_masks=None, point=None): 
        # ref_mask: [w, h]
        chosen_masks = []
        # print("Choose mask original")
        merge_idx = []
    ...
    def update_custom_masks(self, branch_idx, step=None): 
        ...
        if branch_idx in list(range(len(self.all_token_ids))):
            ...
            if self.accum:
                n_clusters_list = [num_clusters_min ** 2]
            else:
                n_clusters_list = list(range(num_clusters_min, num_clusters_max))  
            
            # print("Update custom masks", n_clusters_list, flush=True)     
            ...
            del old_base_masks_1d
            
            if not self.rect_mask:
                if self.accum:
                    batch_custom_masks = self.get_masks_from_attn(mean_custom_attn, [num_clusters_max])
                    chosen_custom_masks, merge_indices_custom = self.choose_mask2(batch_custom_masks, ref_masks=old_custom_masks)
                    del batch_custom_masks

                    batch_base_masks = self.get_masks_from_attn(mean_base_attn, [num_clusters_max])
                    chosen_base_masks, merge_indices_base = self.choose_mask2(batch_base_masks, ref_masks=old_base_masks)
                    del batch_base_masks
                else:
                    ...
                    chosen_custom_masks, merge_indices_custom = self.choose_mask(batch_custom_masks, ref_masks=old_custom_masks)
                    chosen_base_masks, merge_indices_base = self.choose_mask(batch_base_masks, ref_masks=old_base_masks)
            else:
                chosen_custom_masks = []
                ...
                merge_indices_base, merge_indices_custom = None, None
            ...
            if step is not None:
                self.record_info([step, branch_idx, "custom"], merge_indices_custom)
                self.record_info([step, branch_idx, "base"], merge_indices_base)
            
    # Update masks and handle overlaps          
    def refine_feature_masks(self, step=None): 
        
        for branch_idx in range(1, len(self.all_token_ids)):
            self.update_custom_masks(branch_idx, step=step)       
    ...
    def view_self_attn(self, processors_view, self_attn_outdir):#, num_clusters=5):
        ...
        for processor_name in processors_view:
            for num_clusters in n_clusters_list:
                print(num_clusters, flush=True)
                ...
                if ref_attn is not None:
                    ...
                    segment_map_img.save(f'{filename}_{num_clusters}.png')
                
                for branch_idx in range(len(self.all_token_ids)):
                    attn = self.extract(branch_idx, processor_name, 'attention_probs')
                    N = attn.shape[0]
                    print(attn.shape, flush=True)
                    if attn is not None:
                        del attn
                        for i in range(N):
                            attn = self.extract(branch_idx, processor_name, 'attention_probs')[i]
                            cluster_labels = self.kmeans_cluster(attn.unsqueeze(0), num_clusters)
                            segment_map_imgs = self.visualize_labels(cluster_labels)

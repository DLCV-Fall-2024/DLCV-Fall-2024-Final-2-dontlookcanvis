...
if [ -z "$1" ]; then
  outimg_dir="./test_output_codalab"
else
  outimg_dir="$1"
fi

# Folder contains prompt 0's config files
prompt0_folder="./configs/inference/prompt0"

for config_file in "$prompt0_folder"/*; do
...
prompt3_folder="./configs/inference/prompt3" #TODO: folder to config of prompt3_submit_lora_alpha_070705; put yaml file under the folder!!!!
